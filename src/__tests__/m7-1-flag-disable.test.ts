...
describe('M7_ENABLED=false: fog gating disabled', () => {
  it('enemy with coverage=0 renders with dot (not hidden) when m7Enabled=false', () => {
  ...
  it('enemy with coverage=0 renders with manpower when m7Enabled=false', () => {
    const { ctx } = makeMockCtx()
    const args = makeArgs({ coverage: 0, m7Enabled: false })
    callDraw(ctx, args)
  ...
  it('enemy with coverage=30 renders with manpower when m7Enabled=false', () => {
    const { ctx } = makeMockCtx()
    const args = makeArgs({ coverage: 30, m7Enabled: false })
    callDraw(ctx, args)
    expect(ctx.arc).toHaveBeenCalled()
    expect(ctx.fillText).toHaveBeenCalled()
  })

  it('m7Enabled=false does not affect own realm (own still renders fully)', () => {
    const { ctx } = makeMockCtx()
    const args = makeArgs({ isOwnRealm: true, coverage: 0, m7Enabled: false })
    callDraw(ctx, args)
    expect(ctx.arc).toHaveBeenCalled()
    expect(ctx.fillText).toHaveBeenCalled()
  })

  it('m7Enabled=false: multiple enemies with 0 coverage all render', () => {
    const { ctx } = makeMockCtx()
    const ownArmy = {
      id: 'army_own',
      realmId: PLAYER,
