...
const BUTTONS = [
  ...
  { id: 'neizheng', label: '内政', enabled: false },
...
export function BottomBar({ onWanggong, onJunshi }: BottomBarProps) {
  return (
    <nav className={styles.bar} data-testid="bottom-bar">
      {BUTTONS.map(({ id, label, enabled }) => (
        <button
          ...
          onClick={enabled ? (id === 'wanggong' ? onWanggong : onJunshi) : undefined}
        >
          {label}
        </button>
      ))}
