...
- [ ] 11. **架构强制规则（ESLint no-restricted-imports + Engine purity 测试）**
...
- [ ] 6. **时钟（advanceClock 纯函数 + Tick phases 数组）**
...
- [ ] 8. **WorldState 工厂 + 加载 m0 数据 + Zod 校验**
...
- [ ] 14. **应用入口与组装（重写 main.tsx 占位 + App.tsx + 演示完成提示）**
...
- [ ] 15. **E2E 测试套件 + 交付物（Playwright video + 截图 + 完整 QA 矩阵）**
...
### Final Checklist
- [ ] 所有 "Must Have"（17 项）实现存在、可运行、可验证
- [ ] 所有 "Must NOT Have" 不存在（agent 用 grep + AST 扫描确认）
- [ ] 所有 18 项 QA-* 验收标准通过
- [ ] `artifacts/m0-demo.webm` 存在 + 35s+
- [ ] 3 张截图齐全
- [ ] 主创团队观看视频后回答："是的，这个手感就是我们要的"
- [ ] M0 Go/No-Go 评审通过 → 准备进入 M1
